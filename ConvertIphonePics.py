...
# The zip file directory is set as zip_dir_path. The extracted file directory is set as extract_dir_path. 
# The converted file directory is set as convert_dir_path.
...
for filename in os.listdir(zip_dir_path):
    ...
    if fnmatch.fnmatch(filename, zip_pattern):
        try:
            ...
            print(f"Error: {e}")
            print(f"Failed to extract {filename} to {extract_dir_path} ")
...
for filename in os.listdir(extract_dir_path):
    if fnmatch.fnmatch(filename, "*.heic"):
        ...
        print(f"File {filename} is not a HEIC file")
